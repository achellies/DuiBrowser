...
static ATOM registerPopup()
{
	...
	wc.cbClsExtra     = 0;
	wc.cbWndExtra     = sizeof(PopupMenu*); // For the PopupMenu pointer
	wc.hInstance      = Page::instanceHandle();
	wc.hIcon          = 0;
	wc.hCursor        = LoadCursor(0, IDC_ARROW);
	wc.hbrBackground  = 0;
	wc.lpszMenuName   = 0;
	wc.lpszClassName  = kPopupWindowClassName;
	...
	case WM_KEYDOWN:
		if (popup && popup->client()) {
			...
			case VK_TAB:
				...
				if (::GetCapture() == popup->popupHandle())
					::ReleaseCapture(); // Modify by wangwei
				popup->client()->hidePopup();
				break;
			default:
