...
#ifdef WIN32
		memset(&tok->tid, 0, sizeof(pthread_t));
#else
        tok->tid = 0;
#endif
    }
...
xmlGetThreadId(void)
{
    ...
    if (libxml_is_threaded == 0)
        return(0);
#if WIN32
	return((int)(pthread_getw32threadid_np(pthread_self())));
#else
...
#ifdef HAVE_PTHREAD_H
#ifdef WIN32
	return(pthread_getw32threadhandle_np(mainthread) == pthread_getw32threadhandle_np(pthread_self()));
#else
