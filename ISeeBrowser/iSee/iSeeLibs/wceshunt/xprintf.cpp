    ...
    case FORMAT_DOUBLE:
      {
        ...
        if(width >= 0) {
          /* RECURSIVE USAGE */
          len = xsnprintf(fptr, left, "%ld", width);
          fptr += len;
          left -= len;
        }
        if(prec >= 0) {
          /* RECURSIVE USAGE */
          len = xsnprintf(fptr, left, ".%ld", prec);
          fptr += len;
          left -= len;
        }
...
int xvsnprintf(char *buffer, size_t maxlength, const char *format,
                    va_list ap_save)
{
  int retcode;
...
int xsnprintf(char *buffer, size_t maxlength, const char *format, ...)
{
  ...
  retcode = xvsnprintf(buffer, maxlength, format, ap_save);
  va_end(ap_save);
  return retcode;
}
...
int xscprintf(const char *format, ...)
{
  ...
  va_list ap_save; /* argument pointer */
  va_start(ap_save, format);
  retcode = xvscprintf(format, ap_save);
  va_end(ap_save);
  return retcode;
}
