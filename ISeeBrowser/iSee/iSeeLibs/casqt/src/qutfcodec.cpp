...
QByteArray QUtf8Codec::convertFromUnicode(const ushort* uc, int len, ConverterState* state)
{
    uchar replacement = '?';
    int rlen = 3*len;
...
void QUtf8Codec::convertToUnicode(QString *target, const char *chars, int len, ConverterState *state)
{
    ...
    ushort *qch = result.data() + originalLength;
    uchar ch;
    int invalid = 0;
...
QString QUtf8Codec::convertToUnicode(const char *chars, int len, ConverterState *state)
{
    QString result;
...
QByteArray QUtf16Codec::convertFromUnicode(const ushort* uc, int len, ConverterState* state)
{
    Endianness endian = e;
    int length =  2*len;
    if (!state || (!(state->flags & IgnoreHeader))) {
...
QString QUtf16Codec::convertToUnicode(const char *chars, int len, ConverterState *state)
{
    Endianness endian = e;
    bool half = false;
    uchar buf = 0;
    ...
    while (len--) {
        if (half) {
            ...
            if (!headerdone) {
                if (endian == Detect) {
					if (ch.unicode() == QChar::ByteOrderSwapped && endian != BE) {
                        endian = LE;
					} else if (ch.unicode() == QChar::ByteOrderMark && endian != LE) {
                        // ignore BOM
                        endian = BE;
                    } else {
				...
				} else if (ch.unicode() != QChar::ByteOrderMark) {
					*qch++ = ch.unicode();
                }
                headerdone = true;
            } else {
...
const Aliase* QUtf16Codec::aliases()
{
	if (!m_aliases)
	{
		m_aliases = new Aliase[2];
		m_aliases[0] = "ISO-10646-UCS-2";
		m_aliases[1] = 0;
	}
	return m_aliases;
}
...
QByteArray QUtf32Codec::convertFromUnicode(const ushort* uc, int len, ConverterState* state)
{
    Endianness endian = e;
    int length =  4*len;
    if (!state || (!(state->flags & IgnoreHeader))) {
    ...
    if (endian == BE) {
        for (int i = 0; i < len; ++i) {
            uint cp = uc[i];
            if (QChar(uc[i]).isHighSurrogate() && i < len - 1)
                cp = QChar::surrogateToUcs4(cp, uc[++i]);
            *(data++) = cp >> 24;
            *(data++) = (cp >> 16) & 0xff;
            *(data++) = (cp >> 8) & 0xff;
            *(data++) = cp & 0xff;
...
QString QUtf32Codec::convertToUnicode(const char *chars, int len, ConverterState *state)
{
    Endianness endian = e;
    uchar tuple[4];
    int num = 0;
