...
static const char * const Esc_SEQ[]          = { Esc_Ascii,
                                                 ...
                                                 Esc_JISX0208_1983,
                                                 Esc_JISX0212 };
#define SetCurosr(cursor, EscChars) \
do { const char *c=EscChars; cursor[0] = c[0]; cursor[1] = c[1]; cursor[2] = c[2]; if (c[3]) cursor[3] = c[3]; } while(0)
...
QByteArray QJisCodec::convertFromUnicode(const ushort* uc, int len, ConverterState* cs)
{
    char replacement = '?';
    if (cs) {
    ...
    QByteArray result;
	result.resize((len<<1)+1);
    uchar* cursor = (uchar*)result.data();
    ...
    for (int i = 0; i < len; i++) {
        ...
        if (state != prev) {
            if (state == UnknownState) {
				SetCurosr(cursor, Esc_Ascii);
            } else {
				SetCurosr(cursor, Esc_SEQ[state - MinState]);
            }
            prev = state;
        }
        if (j < 0x0100) {
	...
	result.resize(cursor - (uchar*)result.constData());
...
QString QJisCodec::convertToUnicode(const char* chars, int len, ConverterState *cs)
{
    uchar buf[4] = {0, 0, 0, 0};
    int nbuf = 0;
            ...
            if (ch == Esc) {
                ...
                case 0:
                    ...
                    case JISX0212:
                        ...
                        break;
                    default:
                        *cursor++ = ushort(QChar::ReplacementCharacter);
                        break;
                    }
                    break;
                case 1:
...
const Aliase* QJisCodec::aliases()
{
	if (!m_aliases)
	{
