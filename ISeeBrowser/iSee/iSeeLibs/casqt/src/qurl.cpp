...
static QString qt_nameprep(const ushort* source, int size)
{
    // Characters commonly mapped to nothing are simply removed
    // (Table B.1)
    QString mapped;
    for (int i = 0; i < size; ++i) {
        if (!isMappedToNothing(source[i]))
            mapped += source[i];
    }
...
*/
static QByteArray toPunycode(const QString &uc)
{
    uint n = initial_n;
    uint delta = 0;
...
QByteArray QString::toAce(const QString &domain)
{
    // IDNA / rfc3490 describes these four delimiters used for
    // separating labels in unicode international domain
    // names.
    const ushort* data = domain.unicode();
    const ushort* end = data + domain.length();
    const ushort* start = data;
    QByteArray result;
    
    for (const ushort* p = data; p < end; p++)
    {
    	if (*p == 0x2e || *p == 0x3002 || *p == 0xff0e || *p == 0xff61)
		{
			if (start != data)  result += '.';
			result += toPunycode(qt_nameprep(start, p - start));
			start = p + 1;
		}
    }
	...
	{
		if (start != data)  result += '.';
		result += toPunycode(qt_nameprep(start, end - start));
	}
