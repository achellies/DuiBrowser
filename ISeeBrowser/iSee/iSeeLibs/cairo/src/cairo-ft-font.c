 ...
 *
 * Copyright 漏 2000 Keith Packard
 * Copyright 漏 2005 Red Hat, Inc
 *
...
#ifdef NO_FONTCONFIG
_cairo_ft_unscaled_font_create_for_fileid (char *filename, int id)
#else
				   ...
				   cairo_matrix_t	      *scale)
{
	...
	{
		sf.x_scale = unscaled->x_scale;
		sf.y_scale = unscaled->y_scale;
	}
#endif
				  ...
				  cairo_scaled_font_t	     **font)
{
    ...
    cairo_ft_font_transform_t sf;
    cairo_ft_options_t ft_options;
    char *file = getenv((char *)toy_face->family);
	int id = 0;

	// 需要根据字体的附加参数，如果粗体、斜体、字体大小等获取相应的Face
	// 目前暂不实现
	// TODO:主要获取id信息
    ...
    unscaled = _cairo_ft_unscaled_font_create_for_fileid (file, id);
    if (!unscaled)
		goto NOW_RETURN;
