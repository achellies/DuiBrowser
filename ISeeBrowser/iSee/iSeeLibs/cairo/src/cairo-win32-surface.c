		       ...
		       int                   *rowstride_out)
{
	...
	case CAIRO_FORMAT_RGB16_565: /* Fixed for Windows CE 16bits. */
	num_palette = 0;
	break;
	...
	bitmap_info->bmiHeader.biSizeImage = height * ((width * 2 + 3) & ~3);
	bitmap_info->bmiHeader.biClrUsed = 0;	/* unused */
	bitmap_info->bmiHeader.biClrImportant = 0;
				      ...
				      int			num_rects)
{
    ...
    if (surface->format != CAIRO_FORMAT_RGB24 &&
		surface->format != CAIRO_FORMAT_RGB16_565)
	return CAIRO_INT_STATUS_UNSUPPORTED;
...
void
cairo_win32_surface_scroll_dc (cairo_surface_t *surface, int dx, int dy,
							   LPCRECT scrollRect, LPCRECT clipRect, LPRECT updateRect)
{
	cairo_win32_surface_t *winsurf;
	...
	winsurf = (cairo_win32_surface_t *) surface;
	SaveDC(winsurf->dc);
	SelectClipRgn(winsurf->dc, NULL);
	ScrollDC(winsurf->dc, dx, dy, scrollRect, clipRect, NULL, updateRect);
	RestoreDC(winsurf->dc, -1);
}
