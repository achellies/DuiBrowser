 ...
 *
 * Copyright 漏 2000 Keith Packard, member of The XFree86 Project, Inc.
...
/*
 * In the radial gradient problem we are given two circles (c鈧?r鈧? and
 * (c鈧?r鈧? that define the gradient itself. Then, for any point p, we
 ...
 *      c鈧
 *
 * Given (c鈧?r鈧?, (c鈧?r鈧? and p, we must find an angle 胃 such that two
 * points p鈧?and p鈧?on the two circles are collinear with p. Then, the
 * desired value of t is the ratio of the length of p鈧乸 to the length
 * of p鈧乸鈧?
 *
 * So, we have six unknown values: (p鈧亁, p鈧亂), (p鈧倄, p鈧倅), 胃 and t.
 * We can also write six equations that constrain the problem:
 *
 * Point p鈧?is a distance r鈧?from c鈧?at an angle of 胃:
 *
 *	1. p鈧亁 = c鈧亁 + r鈧伮穋os 胃
 *	2. p鈧亂 = c鈧亂 + r鈧伮穝in 胃
 *
 ...
 *	4. p鈧倅 = c鈧倅 + r2路sin 胃
 *
 * Point p lies at a fraction t along the line segment p鈧乸鈧?
 *
 *	5. px = t路p鈧倄 + (1-t)路p鈧亁
 *	6. py = t路p鈧倅 + (1-t)路p鈧亂
 *
 ...
 *
 * px = t路(c鈧倄 + r鈧偮穋os 胃) + (1-t)路(c鈧亁 + r鈧伮穋os 胃)
 * py = t路(c鈧倅 + r鈧偮穝in 胃) + (1-t)路(c鈧亂 + r鈧伮穝in 胃)
 *
 * Then solve each for cos 胃 and sin 胃 expressed as a function of t:
 *
 * cos 胃 = (-(c鈧倄 - c鈧亁)路t + (px - c鈧亁)) / ((r鈧?r鈧?路t + r鈧?
 * sin 胃 = (-(c鈧倅 - c鈧亂)路t + (py - c鈧亂)) / ((r鈧?r鈧?路t + r鈧?
 *
 ...
 * are now:
 *
 * cos 胃 = (-cdx路t + pdx) / (dr路t + r鈧?
 * sin 胃 = (-cdy路t + pdy) / (dr路t + r鈧?
 *
 * Finally, to get a single function of t and eliminate the last
 * unknown 胃, we use the identity sin虏胃 + cos虏胃 = 1. First, square
 * each equation, (we knew a quadratic was coming since it must be
 * possible to obtain two solutions in some cases):
 *
 * cos虏胃 = (cdx虏t虏 - 2路cdx路pdx路t + pdx虏) / (dr虏路t虏 + 2路r鈧伮穌r路t + r鈧伮?
 * sin虏胃 = (cdy虏t虏 - 2路cdy路pdy路t + pdy虏) / (dr虏路t虏 + 2路r鈧伮穌r路t + r鈧伮?
 *
 * Then add both together, set the result equal to 1, and express as a
 * standard quadratic equation in t of the form At虏 + Bt + C = 0
 *
 * (cdx虏 + cdy虏 - dr虏)路t虏 - 2路(cdx路pdx + cdy路pdy + r鈧伮穌r)路t + (pdx虏 + pdy虏 - r鈧伮? = 0
 *
 * In other words:
 *
 * A = cdx虏 + cdy虏 - dr虏
 * B = -2路(pdx路cdx + pdy路cdy + r鈧伮穌r)
 * C = pdx虏 + pdy虏 - r鈧伮
 *
