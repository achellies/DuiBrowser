...
int HTMLParser::m_st_iIsEmpty = 1;
int HTMLParser::m_st_iIsClearing = 0;

HashSet<AtomicStringImpl*>* HTMLParser::unaffectedTagsList()
{
    static HashSet<AtomicStringImpl*> unaffectedTags;
    if ((unaffectedTags.isEmpty())&&(!m_st_iIsClearing)) 
	{
		m_st_iIsEmpty = 0;
    ...
    return &unaffectedTags;
}

FunctionMap* HTMLParser::gFunctionMapList()
{
    // Init our error handling table.
    static FunctionMap gFunctionMap;
    if ((gFunctionMap.isEmpty())&&(!m_st_iIsClearing)) 
	{
		m_st_iIsEmpty = 0;
...
HashSet<AtomicStringImpl*>* HTMLParser::headerTagsList()
{
    static HashSet<AtomicStringImpl*> headerTags;
    if ((headerTags.isEmpty())&&(!m_st_iIsClearing))
	{
		m_st_iIsEmpty = 0;
...
HashSet<AtomicStringImpl*>* HTMLParser::residualStyleTagsList()
{
    static HashSet<AtomicStringImpl*> residualStyleTags;
    if ((residualStyleTags.isEmpty())&&(!m_st_iIsClearing)) 
	{
		m_st_iIsEmpty = 0;
	...
	return &residualStyleTags;
}

void HTMLParser::staticFinalize()
{
	if (!m_st_iIsEmpty)
	{
		m_st_iIsClearing = 1;
		unaffectedTagsList()->clear();
		gFunctionMapList()->clear();
		headerTagsList()->clear();
		residualStyleTagsList()->clear();
		m_st_iIsEmpty = 1;
		m_st_iIsClearing = 0;
	}
}
...
PassRefPtr<Node> HTMLParser::getNode(Token* t)
{
    ...
    bool proceed = true;
    RefPtr<Node> result;
    if (CreateErrorCheckFunc errorCheckFunc = gFunctionMapList()->get(t->tagName.impl()))
	{
...
{
//+daw ca 23/07 static and global management
    return headerTagsList()->contains(tagName.impl());
//daw ca
}
...
{
//+daw ca 23/07 static and global management
    return residualStyleTagsList()->contains(tagName.impl());
// daw ca
}
...
{
//+daw ca 23/07 static and global management
    return !(unaffectedTagsList()->contains(tagName.impl()));
//daw ca
}
