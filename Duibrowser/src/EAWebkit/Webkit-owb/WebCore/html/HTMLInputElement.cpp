...
{
    /*NVK:  if you want to have radio and text buttons react to a
    mouse hover, you heed this bit of code
    if (isTextField() || isTextButton() || isRadioButton())
    
        Also modify Document::setHoverNode() in Document.cpp
    */
...
//+daw ca 28/07 static and global management, move static definition to AtomicString
const AtomicString& HTMLInputElement::type() const
{
    // needs to be lowercase according to DOM spec
    switch (inputType()) {
        case BUTTON: {
            return buttonHTMLInputElement;
        }
        case CHECKBOX: {
            return checkboxHTMLInputElement;
        }
        case FILE: {
            return fileHTMLInputElement;
        }
        case HIDDEN: {
            return hiddenHTMLInputElement;
        }
        case IMAGE: {
            return imageHTMLInputElement;
        }
        case ISINDEX:
            return emptyAtom;
        case PASSWORD: {
            return passwordHTMLInputElement;
        }
        case RADIO: {
            return radioHTMLInputElement;
        }
        case RANGE: {
            return rangeHTMLInputElement;
        }
        case RESET: {
            return resetHTMLInputElement;
        }
        case SEARCH: {
            return searchHTMLInputElement;
        }
        case SUBMIT: {
            return submitHTMLInputElement;
        }
        case TEXT: {
            return textHTMLInputElement;
        }
    }
    return emptyAtom;
}
...
void HTMLInputElement::parseMappedAttribute(MappedAttribute *attr)
{
    if (attr->name() == nameAttr) {
        ...
        setHTMLEventListener(eventNames().selectEvent, attr);
    } else if (attr->name() == onchangeAttr) {
        setHTMLEventListener(eventNames().changeEvent, attr);
    } else if (attr->name() == oninputAttr) {
        setHTMLEventListener(eventNames().inputEvent, attr);
    }
    // Search field and slider attributes all just cause updateFromElement to be called through style
    // recalcing.
    else if (attr->name() == onsearchAttr) {
        setHTMLEventListener(eventNames().searchEvent, attr);
...
void HTMLInputElement::setValueFromRenderer(const String& value)
{
    ...
    dispatchHTMLEvent(eventNames().inputEvent, true, false);
}
...
void HTMLInputElement::onSearch()
{
    ...
    dispatchHTMLEvent(eventNames().searchEvent, true, false);
}
