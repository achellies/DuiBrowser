...
bool HTMLFormElement::prepareSubmit(Event* event)
{
    ...
    if (dispatchHTMLEvent(eventNames().submitEvent, true, true) && !m_doingsubmit)
        m_doingsubmit = true;
...
void HTMLFormElement::reset()
{
    ...
    if ( !dispatchHTMLEvent(eventNames().resetEvent,true, true) ) {
        m_inreset = false;
        return;
    }
...
void HTMLFormElement::parseMappedAttribute(MappedAttribute* attr)
{
    if (attr->name() == actionAttr)
        ...
        setHTMLEventListener(eventNames().resetEvent, attr);
