...
void HTMLBodyElement::parseMappedAttribute(MappedAttribute *attr)
{
    if (attr->name() == backgroundAttr) {
        ...
        document()->setHTMLWindowEventListener(eventNames().loadEvent, attr);
    } else if (attr->name() == onbeforeunloadAttr) {
        document()->setHTMLWindowEventListener(eventNames().beforeunloadEvent, attr);
    } else if (attr->name() == onunloadAttr) {
        document()->setHTMLWindowEventListener(eventNames().unloadEvent, attr);
    } else if (attr->name() == onblurAttr) {
        document()->setHTMLWindowEventListener(eventNames().blurEvent, attr);
    } else if (attr->name() == onfocusAttr) {
        document()->setHTMLWindowEventListener(eventNames().focusEvent, attr);
    } else if (attr->name() == onresizeAttr) {
        document()->setHTMLWindowEventListener(eventNames().resizeEvent, attr);
    } else if (attr->name() == onscrollAttr) {
        document()->setHTMLWindowEventListener(eventNames().scrollEvent, attr);
        ...
        setHTMLEventListener(eventNames().storageEvent, attr);
    } else
