...
DOMImplementation* DOMImplementation::pMakeInstance = NULL;

void DOMImplementation::staticFinalize()
{
    delete pMakeInstance;
}
//daw ca
...
bool DOMImplementation::isXMLMIMEType(const String& mimeType)
{
    ...
    // 4/23/09 CSidhall - Removed as static to fix memory leak since just used to return a bool flag 
    // and probably not called often (4 times during load on a test site).      
    //- Original code:
    // static RegularExpression xmlTypeRegExp(String("^") + validChars + "+/" + validChars + "+\\+xml$");
    //- New code:
    RegularExpression xmlTypeRegExp(String("^") + validChars + "+/" + validChars + "+\\+xml$");
    //-
