...
// 3/27//09 CSidhall - Added for memory leak on exit
void StyledElement::staticFinalize()
{
    if (mappedAttributeDecls){
        deleteAllValues(*mappedAttributeDecls); // 4/15/09 CS - Needed for leak fix
        delete mappedAttributeDecls;
        mappedAttributeDecls = NULL;
    }
}   
