...
// Setup by Paul Pedriana, 1/2009 to fix a memory leak.
static ElementRareDataMap* gDataMap = 0;

void Element::staticFinalize()
{
    delete gDataMap; // OK if NULL.
    gDataMap = 0;
}
...
{
    if(!gDataMap)
        gDataMap = new ElementRareDataMap;
    return *gDataMap;
}
...
Element::~Element()
{
    ...
    if (!m_attrWasSpecifiedOrElementHasRareData)
        ASSERT(!gDataMap || !rareDataMap().contains(this));
    else {
...
KURL Element::baseURI() const
{
    ...
    KURL base(getAttribute(xmlNames->baseAttr));
...
RenderStyle* Element::computedStyle()
{
    ...
    ElementRareData* data = createRareData();
