...
static void clearWrapper();         // 3/30/09 CSidhall - Added for leak fix
...
static inline void clearWrapper()
{
}
...
void ScriptInterpreter::staticFinalize()
{
    DOMObjectMap &DOMObjMap =domObjects();  // 3/31/09 CS - Added missing ref pointer. 
    DOMObjMap.clear();

    clearWrapper();         // 3/30/09 CSidhall - Added for exit leak
}
