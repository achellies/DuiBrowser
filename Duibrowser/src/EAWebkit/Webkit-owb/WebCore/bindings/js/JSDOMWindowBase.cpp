...
JSValue *JSDOMWindowBase::getValueProperty(ExecState *exec, int token) const
{
   ...
   switch (token) {
   case Onabort:
     return getListener(exec, eventNames().abortEvent);
   case Onblur:
     return getListener(exec, eventNames().blurEvent);
   case Onchange:
     return getListener(exec, eventNames().changeEvent);
   case Onclick:
     return getListener(exec, eventNames().clickEvent);
   case Ondblclick:
     return getListener(exec, eventNames().dblclickEvent);
   case Onerror:
     return getListener(exec, eventNames().errorEvent);
   case Onfocus:
     return getListener(exec, eventNames().focusEvent);
   case Onkeydown:
     return getListener(exec, eventNames().keydownEvent);
   case Onkeypress:
     return getListener(exec, eventNames().keypressEvent);
   case Onkeyup:
     return getListener(exec, eventNames().keyupEvent);
   case Onload:
     return getListener(exec, eventNames().loadEvent);
   case Onmousedown:
     return getListener(exec, eventNames().mousedownEvent);
   case Onmousemove:
     return getListener(exec, eventNames().mousemoveEvent);
   case Onmouseout:
     return getListener(exec, eventNames().mouseoutEvent);
   case Onmouseover:
     return getListener(exec, eventNames().mouseoverEvent);
   case Onmouseup:
     return getListener(exec, eventNames().mouseupEvent);
   case OnWindowMouseWheel:
     return getListener(exec, eventNames().mousewheelEvent);
   case Onreset:
     return getListener(exec, eventNames().resetEvent);
   case Onresize:
     return getListener(exec, eventNames().resizeEvent);
   case Onscroll:
     return getListener(exec, eventNames().scrollEvent);
   case Onsearch:
     return getListener(exec, eventNames().searchEvent);
   case Onselect:
     return getListener(exec, eventNames().selectEvent);
   case Onsubmit:
     return getListener(exec, eventNames().submitEvent);
   case Onbeforeunload:
     return getListener(exec, eventNames().beforeunloadEvent);
   case Onunload:
     return getListener(exec, eventNames().unloadEvent);
   }
   ASSERT_NOT_REACHED();
   return jsUndefined();
}
...
void JSDOMWindowBase::put(ExecState* exec, const Identifier& propertyName, JSValue* value)
{
  ...
  if (entry) {
    ...
    case Onabort:
      if (allowsAccessFrom(exec))
        setListener(exec, eventNames().abortEvent,value);
      return;
    case Onblur:
      if (allowsAccessFrom(exec))
        setListener(exec, eventNames().blurEvent,value);
      return;
    case Onchange:
      if (allowsAccessFrom(exec))
        setListener(exec, eventNames().changeEvent,value);
      return;
    case Onclick:
      if (allowsAccessFrom(exec))
        setListener(exec, eventNames().clickEvent,value);
      return;
    case Ondblclick:
      if (allowsAccessFrom(exec))
        setListener(exec, eventNames().dblclickEvent,value);
      return;
    case Onerror:
      if (allowsAccessFrom(exec))
        setListener(exec, eventNames().errorEvent, value);
      return;
    case Onfocus:
      if (allowsAccessFrom(exec))
        setListener(exec, eventNames().focusEvent,value);
      return;
    case Onkeydown:
      if (allowsAccessFrom(exec))
        setListener(exec, eventNames().keydownEvent,value);
      return;
    case Onkeypress:
      if (allowsAccessFrom(exec))
        setListener(exec, eventNames().keypressEvent,value);
      return;
    case Onkeyup:
      if (allowsAccessFrom(exec))
        setListener(exec, eventNames().keyupEvent,value);
      return;
    case Onload:
      if (allowsAccessFrom(exec))
        setListener(exec, eventNames().loadEvent,value);
      return;
    case Onmousedown:
      if (allowsAccessFrom(exec))
        setListener(exec, eventNames().mousedownEvent,value);
      return;
    case Onmousemove:
      if (allowsAccessFrom(exec))
        setListener(exec, eventNames().mousemoveEvent,value);
      return;
    case Onmouseout:
      if (allowsAccessFrom(exec))
        setListener(exec, eventNames().mouseoutEvent,value);
      return;
    case Onmouseover:
      if (allowsAccessFrom(exec))
        setListener(exec, eventNames().mouseoverEvent,value);
      return;
    case Onmouseup:
      if (allowsAccessFrom(exec))
        setListener(exec, eventNames().mouseupEvent,value);
      return;
    case OnWindowMouseWheel:
      if (allowsAccessFrom(exec))
        setListener(exec, eventNames().mousewheelEvent,value);
      return;
    case Onreset:
      if (allowsAccessFrom(exec))
        setListener(exec, eventNames().resetEvent,value);
      return;
    case Onresize:
      if (allowsAccessFrom(exec))
        setListener(exec, eventNames().resizeEvent,value);
      return;
    case Onscroll:
      if (allowsAccessFrom(exec))
        setListener(exec, eventNames().scrollEvent,value);
      return;
    case Onsearch:
        if (allowsAccessFrom(exec))
            setListener(exec, eventNames().searchEvent,value);
        return;
    case Onselect:
      if (allowsAccessFrom(exec))
        setListener(exec, eventNames().selectEvent,value);
      return;
    case Onsubmit:
      if (allowsAccessFrom(exec))
        setListener(exec, eventNames().submitEvent,value);
      return;
    case Onbeforeunload:
      if (allowsAccessFrom(exec))
        setListener(exec, eventNames().beforeunloadEvent, value);
      return;
    case Onunload:
      if (allowsAccessFrom(exec))
        setListener(exec, eventNames().unloadEvent, value);
      return;
    default:
      break;
    }
