...
// 4/17/09 CSidhall - Added for leak fix
void RenderBox::staticFinalize()
{
    if(gOverrideSizeMap) {
        delete gOverrideSizeMap;
        gOverrideSizeMap = NULL;
    }
}
...
void RenderBox::styleDidChange(RenderStyle::Diff diff, const RenderStyle* oldStyle)
{
    ...
    if (hasOverflowClip() && oldStyle && style() && oldStyle->effectiveZoom() != style()->effectiveZoom()) {
        int left = scrollLeft();
        if (left) {
            left = (int) ((left / oldStyle->effectiveZoom()) * style()->effectiveZoom());
            setScrollLeft(left);
        }
        int top = scrollTop();
        if (top) {
            top = (int) ((top / oldStyle->effectiveZoom()) * style()->effectiveZoom());
            setScrollTop(top);
        }
    }
