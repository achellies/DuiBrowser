...
//+ CSidhall 4/17/09 - Added for possible memory leak 
HashMap<RenderBlock*, RenderFlowSequencedSet*>* continuationOutlineTable();
void RenderBlock::staticFinalize()
{
    if(gColumnInfoMap){
        // CS - Need to verify if a deleteAllvalues might also be needed here...
        delete gColumnInfoMap;
        gColumnInfoMap = NULL;
    }

    // Added on 4/27/09 as a possible leak 
    HashMap<RenderBlock*, RenderFlowSequencedSet*>*  pTable = continuationOutlineTable();
    if(pTable) {
        deleteAllValues(*pTable);
    }
}
//- CS
