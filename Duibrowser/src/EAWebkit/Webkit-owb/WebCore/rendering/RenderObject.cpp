                              ...
                              int adjbw1, int adjbw2)
{
    ...
    switch (style) {
        ...
        case DASHED:
            ...
            break;
        case DOUBLE: {
            graphicsContext->save(); //+ 10/7/09 CSidhall - Added for text underline bug
        ...
        case OUTSET:
            ...
            // fall through
        case SOLID: {
            graphicsContext->save();    //+ 10/7/09 CSidhall - added for text underline context bug
            graphicsContext->setStrokeStyle(NoStroke);
            graphicsContext->setFillColor(c);
            ASSERT(x2 >= x1);
            ASSERT(y2 >= y1);
...
{
    if (!isText() && style)
        setStyle(animation()->updateImplicitAnimations(this, style.get()));
    else
        setStyle(style);
}
...
{
    if (m_style == style || !style.get()) //AJB: Added (!style.get() )to get rid of a crash on loading myspace. Chris said that it is probably possible for the style to be NULL if the site can not provide it
        return;
