...
#include <EAWebKit/internal/EAWebKitSort.h>   
...
void RenderLayer::beginTransparencyLayers(GraphicsContext* p, const RenderLayer* rootLayer)
{
    ...
    if (isTransparent()) {
        ...
        // 3/13/09 CSidhall - Removed this clip set for it ignored the dirty rect and current draw clip which cause problems with widgets for example on ea.com
        //  p->clip(transparencyClipBox(AffineTransform(), this, rootLayer));
        // Clip against the dirty rect window instead so we stay within
        FloatRect rect = transparencyClipBox(AffineTransform(), this, rootLayer);
        FloatRect clipRect = p->getClip();
        
        // CS - Added scroll adjustment as clip rect is in screen space 
        IntSize size =p->origin();
        rect.setX(rect.x()  + (float) size.width());
        rect.setY(rect.y()  + (float) size.height());
        ...
        rect.setX( rect.x() - size.width());    
        rect.setY( rect.y() - size.height());    
...
void RenderLayer::scrollToOffset(int x, int y, bool updateScrollbars, bool repaint)
{
    ...
    if (view) {
        if (FrameView* frameView = view->frameView())
            frameView->scheduleEvent(Event::create(eventNames().scrollEvent, true, false), EventTargetNodeCast(renderer()->element()), true);
    }
}
...
void RenderLayer::resize(const PlatformMouseEvent& evt, const IntSize& oldOffset)
{
    ...
    if (!document->frame()->eventHandler()->mousePressed(evt.button()))
        return;

    float zoomFactor = renderer->style()->effectiveZoom();

    IntSize newOffset = offsetFromResizeCorner(document->view()->windowToContents(evt.pos()));
    newOffset.setWidth(static_cast<int>(newOffset.width() / zoomFactor));
    newOffset.setHeight(static_cast<int>(newOffset.height() / zoomFactor));
    
    IntSize currentSize = IntSize(static_cast<int>(renderer->width() / zoomFactor), static_cast<int>(renderer->height() / zoomFactor));
    IntSize minimumSize = element->minimumSizeForResizing().shrunkTo(currentSize);
    element->setMinimumSizeForResizing(minimumSize);
    
    IntSize adjustedOldOffset = IntSize(static_cast<int>(oldOffset.width() / zoomFactor), static_cast<int>(oldOffset.height() / zoomFactor));
    ...
    if (difference.width()) {
        ...
        baseWidth = static_cast<int>(baseWidth / zoomFactor);
    ...
    if (difference.height()) {
        ...
        baseHeight = static_cast<int>(baseHeight / zoomFactor);
                        ...
                        RenderObject* paintingRoot, bool appliedTransform)
{
    ...
    if (m_transform && !appliedTransform) {
        ...
        // 2/11/09 CSidhall - moved this after the context save so that we can restore the parent clip rect
...
void RenderLayer::updateZOrderLists()
{
    ...
    //if (m_posZOrderList)
    //    eastl::merge_sort(m_posZOrderList->begin(),m_posZOrderList->end(),allocator,compareZIndex);
    //if (m_negZOrderList)
    //    eastl::merge_sort(m_negZOrderList->begin(), m_negZOrderList->end(),allocator,compareZIndex);

    // New code:
    if (m_posZOrderList) 
        EA::Internal::merge_sort(m_posZOrderList->data(),m_posZOrderList->size(),compareZIndex);
    if (m_negZOrderList)
        EA::Internal::merge_sort(m_negZOrderList->data(),m_negZOrderList->size(),compareZIndex);
    //- CS
