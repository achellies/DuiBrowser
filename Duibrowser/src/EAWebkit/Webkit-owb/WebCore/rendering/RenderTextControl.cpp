...
void RenderTextControl::setStyle(PassRefPtr<RenderStyle> _style)
{
...
void RenderTextControl::updatePlaceholder()
{
    ...
    if (style()->textSecurity() != TSNONE && oldPlaceholderVisible != m_placeholderVisible) {
        RefPtr<RenderStyle> newInnerStyle = RenderStyle::create();   
...
//Nicki Vankoughnett:  This function underwent significant changes by changelist 37637.  Several
//of the classes (such as HTMLTextFieldInnerElement) have been entirely renamed, and much of the
//setrender/setattached/setindocument calls had been offloaded to the renamed class.
//It seems that the sanest approach here is to change the types the compiler complains about
...
void RenderTextControl::createSubtreeIfNeeded()
{
    ...
    if (isSearchField && !m_resultsButton) {
        ...
        PassRefPtr<RenderStyle> resultsBlockStyle = createResultsButtonStyle(m_innerBlock->renderer()->style());
    ...
    if (!m_innerText) {
        ...
        PassRefPtr<RenderStyle> textBlockStyle = createInnerTextStyle(parentStyle);
        textBlockRenderer->setStyle(textBlockStyle);
    ...
    if (isSearchField && !m_cancelButton) {
        ...
        PassRefPtr<RenderStyle> closeBlockStyle = createCancelButtonStyle(m_innerBlock->renderer()->style());
...
void RenderTextControl::selectionChanged(bool userTriggered)
{
    ...
    if (Frame* frame = document()->frame())
        if (frame->selection()->isRange() && userTriggered)
            element->dispatchHTMLEvent(eventNames().selectEvent, true, false);
}
...
RenderBlock* RenderTextControl::clientRenderBlock()
{
    return this;
}
