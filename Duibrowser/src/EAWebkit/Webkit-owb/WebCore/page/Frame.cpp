...
Frame::~Frame()
{
    ...
    // 3/31/09 CSidhall - For releasing secondary client webframes to fix memory leaks
    if(m_pClientWebFrame) {
        delete m_pClientWebFrame;
        m_pClientWebFrame = NULL;
    }
}
...
{
    // 7/22/09 CSidhall - Commented out as this is really only to debug draw issues and
    // our debug builds are slow enough without it...
/*
#ifndef NDEBUG
    bool fillWithRed;
...
void Frame::sendScrollEvent()
{
    ...
    if (!doc)
        return;
    doc->dispatchHTMLEvent(eventNames().scrollEvent, true, false);
}
...
//+ 7/23/09 CSidhall added for transparent background support
void Frame::setTransparentBackground(const bool active)
{
    d->m_transparentBackground = active;
}

bool Frame::transparentBackground() const
{
    return d->m_transparentBackground;
}
//- CS
...
FramePrivate::FramePrivate(Page* page, Frame* parent, Frame* thisFrame, HTMLFrameOwnerElement* ownerElement,
    ...
    , m_transparentBackground(false)    // 7/22/09 CS - Added
