...
void FrameView::init()
{
    ...
    m_size = IntSize();
    // 7/22/09 CSidhall - Added to pass down transparent status of main page
    if( (d) && frame() )
        d->m_isTransparent =  frame()->transparentBackground();
}
...
void FrameView::scheduleRelayout()
{
    ...
    if (!m_frame->document()->ownerElement())
       OWB_PRINTF("Scheduling layout for %d\n", delay);
#endif
...
#ifdef INSTRUMENT_LAYOUT_SCHEDULING
    if (m_frame->document() && !m_frame->document()->ownerElement())
       OWB_PRINTF("Layout timer unscheduled at %d\n", m_frame->document()->elapsedTime());
#endif
