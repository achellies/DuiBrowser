...
JSObject* JSCSSStyleDeclarationPrototype::self(ExecState* exec)
{
    ...
    const Identifier prototypeIdentifier(exec, "[[JSCSSStyleDeclaration.prototype]]");
    return KJS::cacheGlobalObject<JSCSSStyleDeclarationPrototype>(exec, prototypeIdentifier);
}
...
{
    // Changed by Paul Pedriana (1/2009), as the static new was creating a memory leak. If this gets called a lot then we can consider making it a static pointer that's freed on shutdown.
    const Identifier constructorIdentifier(exec, "[[CSSStyleDeclaration.constructor]]");
    return KJS::cacheGlobalObject<JSCSSStyleDeclarationConstructor>(exec, constructorIdentifier);
}
...
JSValue* jsCSSStyleDeclarationPrototypeFunctionGetPropertyShorthand(ExecState* exec, JSObject*, JSValue* thisValue, const ArgList& args)
{
    ...
    KJS::JSValue* result = jsStringOrNull(exec, imp->getPropertyShorthand(propertyName));
    return result;
}
