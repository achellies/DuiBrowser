...
JSObject* JSHTMLHeadingElementPrototype::self(ExecState* exec)
{
    ...
    const Identifier prototypeIdentifier(exec, "[[JSHTMLHeadingElement.prototype]]");
    return KJS::cacheGlobalObject<JSHTMLHeadingElementPrototype>(exec, prototypeIdentifier);
}
...
{
    // Changed by Paul Pedriana (1/2009), as the static new was creating a memory leak. If this gets called a lot then we can consider making it a static pointer that's freed on shutdown.
    const Identifier constructorIdentifier(exec, "[[HTMLHeadingElement.constructor]]");
    return KJS::cacheGlobalObject<JSHTMLHeadingElementConstructor>(exec, constructorIdentifier);
}
