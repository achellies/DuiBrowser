...
JSObject* JSCSSFontFaceRulePrototype::self(ExecState* exec)
{
    ...
    const Identifier prototypeIdentifier(exec, "[[JSCSSFontFaceRule.prototype]]");
    return KJS::cacheGlobalObject<JSCSSFontFaceRulePrototype>(exec, prototypeIdentifier);
}
...
{
    // Changed by Paul Pedriana (1/2009), as the static new was creating a memory leak. If this gets called a lot then we can consider making it a static pointer that's freed on shutdown.
    const Identifier constructorIdentifier(exec, "[[CSSFontFaceRule.constructor]]");
    return KJS::cacheGlobalObject<JSCSSFontFaceRuleConstructor>(exec, constructorIdentifier);
}
