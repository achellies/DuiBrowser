...
namespace WTF {
    ...
    using std::max;
            ...
            #define WTF_PREFIX_ALIGN(n)  __declspec(align(n))  // n must be a literal integer, it cannot be a general constant expression.
        ...
        struct aligned_buffer<size, 2>    { WTF_PREFIX_ALIGN(2)  aligned_buffer_char buffer[size] WTF_POSTFIX_ALIGN(2);  };

        template<size_t size>
        struct aligned_buffer<size, 4>    { WTF_PREFIX_ALIGN(4)  aligned_buffer_char buffer[size] WTF_POSTFIX_ALIGN(4);  };

        template<size_t size>
        struct aligned_buffer<size, 8>    { WTF_PREFIX_ALIGN(8)  aligned_buffer_char buffer[size] WTF_POSTFIX_ALIGN(8);  };

        template<size_t size>
        struct aligned_buffer<size, 16>   { WTF_PREFIX_ALIGN(16) aligned_buffer_char buffer[size] WTF_POSTFIX_ALIGN(16); };

        template<size_t size>
        struct aligned_buffer<size, 32>   { WTF_PREFIX_ALIGN(32) aligned_buffer_char buffer[size] WTF_POSTFIX_ALIGN(32); };

        template<size_t size>
        struct aligned_buffer<size, 64>   { WTF_PREFIX_ALIGN(64) aligned_buffer_char buffer[size] WTF_POSTFIX_ALIGN(64); };
