...
QString QBig5Codec::convertToUnicode(const char* chars, int len, ConverterState *state)
{
    ...
    QString result;
	result.resize(len);
	ushort* cursor = (ushort*)result.data();
...
QByteArray QBig5Codec::convertFromUnicode(const ushort* uc, int len, ConverterState* state)
{
    char replacement = '?';
    if (state) {
        if (state->flags & ConvertInvalidToNull)
...
const Aliase* QBig5Codec::aliases()
{
	if (!m_aliases)
	{
		m_aliases = new Aliase[3];
		m_aliases[0] = "Big5-ETen";
		m_aliases[1] = "CP950";
		m_aliases[2] = 0;
	}
	return m_aliases;
...
QString QBig5hkscsCodec::convertToUnicode(const char* chars, int len, ConverterState *state)
{
    uchar buf[2] = {0};
    int nbuf = 0;
    ushort replacement = QChar::ReplacementCharacter;
...
QByteArray QBig5hkscsCodec::convertFromUnicode(const ushort* uc, int len, ConverterState *state)
{
    char replacement = '?';
    if (state) {
        if (state->flags & ConvertInvalidToNull)
