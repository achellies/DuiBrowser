...
QString QSjisCodec::convertToUnicode(const char* chars, int len, ConverterState *state)
{
    uchar buf[1] = {0};
    int nbuf = 0;
...
const Aliase* QSjisCodec::aliases()
{
	if (!m_aliases)
	{
		m_aliases = new Aliase[3];
		m_aliases[0] = "SJIS";
		m_aliases[1] = "MS_Kanji";
		m_aliases[2] = 0;
	}
	return m_aliases;
}
