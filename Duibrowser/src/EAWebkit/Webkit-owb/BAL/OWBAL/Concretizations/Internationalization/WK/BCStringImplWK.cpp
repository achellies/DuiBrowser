...
StringImpl* StringImpl::m_st_pEmptyString = NULL;

StringImpl* StringImpl::empty()
{
    if (!m_st_pEmptyString)
    {
	    m_st_pEmptyString = new StringImpl;
    }
    return m_st_pEmptyString;
}

void StringImpl::staticFinalize()
{
	delete m_st_pEmptyString;
	m_st_pEmptyString = NULL;
}

#if defined(_DEBUG) && defined(_WIN32) && defined(EA_AUTHOR_PPEDRIANA) // Added by Paul Pedriana to help find ref-counting memory leaks.
    void StringImpl::ref()
    {
        if(m_data && !wcsncmp(m_data, L"*", 1))
        {
            // Do something.
            wcsncmp(m_data, L"", 1);  // Do nothing.
        }

        RefCounted<StringImpl>::ref();
    }
