...
static QByteArray *buildReverseMap(int forwardIndex)
{
    ...
    for(i=128; i<256; i++) {
        ...
        if (u < m)
            data[u] = (char)(unsigned char)(i);
    }
    return map;
}

QString QSimpleTextCodec::convertToUnicode(const char* chars, int len, ConverterState *)
{
    if (len <= 0 || chars == 0)
        return QString();
    ...
    for (int i = 0; i < len; i++) {
        if (c[i] > 127)
			...
			uc[i] = QLatin1Char(c[i]).unicode();
    }
    return r;
}

QByteArray QSimpleTextCodec::convertFromUnicode(const ushort* in, int length, ConverterState *state)
{
    const char replacement = (state && state->flags & ConvertInvalidToNull) ? 0 : '?';
    int invalid = 0;

    if (!reverseMap){
        reverseMap = buildReverseMap(this->forwardIndex);
    }
...
const char* QSimpleTextCodec::name()
{
    return unicodevalues[forwardIndex].mime;
}

const Aliase* QSimpleTextCodec::aliases()
{
	if (!m_aliases)
	{
		int cnt;
		for (cnt = 0; unicodevalues[forwardIndex].aliases[cnt]; cnt++);
		m_aliases = new Aliase[cnt + 1];
		for (int i = 0; i < cnt; i++)
			m_aliases[i] = unicodevalues[forwardIndex].aliases[i];
		m_aliases[cnt] = 0;
	}
	return m_aliases;
}
