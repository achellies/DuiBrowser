...
QByteArray QEucKrCodec::convertFromUnicode(const ushort *uc, int len, ConverterState *state)
{
    char replacement = '?';
    if (state) {
...
QString QEucKrCodec::convertToUnicode(const char* chars, int len, ConverterState *state)
{
    uchar buf[2] = {0, 0};
    int nbuf = 0;
...
/*!
  \reimp
*/
QByteArray QCP949Codec::convertFromUnicode(const ushort *uc, int len, ConverterState *state)
{
    char replacement = '?';
    if (state) {
        if (state->flags & ConvertInvalidToNull)
...
/*!
  \reimp
*/
QString QCP949Codec::convertToUnicode(const char* chars, int len, ConverterState *state)
{
    uchar buf[2] = {0, 0};
    int nbuf = 0;
    ushort replacement = QChar::ReplacementCharacter;
            ...
            if (IsEucChar(ch) && !IsCP949Char(buf[0])) {
                ...
                *cursor++ = QValidChar(u);
                ...
                if (internal_code < 0 || internal_code >= 8822) {
                    ...
                    *cursor++ = QValidChar(cp949_icode_to_unicode[internal_code]);
            }
            nbuf = 0;
            break;
        }
