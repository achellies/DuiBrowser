...
#include <assert.h>
#include <stdio.h>
#include "cache.h"              // For image prune
...
bool JPEGImageDecoder::outputScanlines()
{
    ...
    // 7/13/09 CSidhall - Added some RAM cache overflow handling
    ...
    if((buffer.status() == RGBA32Buffer::FrameEmpty) ||(size != curSize) ) {
        // Reserve if possible enough space in the cache + out of memory error handling
        ...
        if(cachePrune == false){
            if(!overflowFlag){
               char buffer[256];
               sprintf(buffer, "RAM Cache Overflow: cache is too small to decode JPEG image: %d bytes\n", (size<< 2) );
    ...
    while (info->output_scanline < info->output_height) {
        ...
        for (unsigned i = 0; i < info->output_width; ++i) {
            ...
            // EA/Alex Mole: JPEGs have no alpha channel, so call the lightweight RGB-only setter
            RGBA32Buffer::setRGBWithPresetAlpha(*dst++, r, g, b);
        }
