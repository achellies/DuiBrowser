...
double getCurrentUTCTimeWithMicroseconds()
{
    ...
    // on non-Unix platforms.
        ...
        /* Windows version that works well.
        FILETIME      ft;
        LARGE_INTEGER li;
...
double parseDate(const UString &date)
{
    ...
    if (dateString == newPosStr || day == LONG_MIN || day == LONG_MAX)
        return NaN;
#else
    errno = 0;
    long day = strtol(dateString, &newPosStr, 10);
    ...
    if (day > 31) {
		...
		// Avoid the use of errno as it is not available on Windows CE
		if (dateString == newPosStr || month == LONG_MIN || month == LONG_MAX)
			return NaN;
#else
        month = strtol(dateString, &newPosStr, 10) - 1;
        if (errno)
    ...
    if (!*newPosStr)
        ...
        if (newPosStr != dateString) {
			...
			// Avoid the use of errno as it is not available on Windows CE
			if (dateString == newPosStr || minute == LONG_MIN || minute == LONG_MAX)
				return NaN;
#else
            minute = strtol(dateString, &newPosStr, 10);
            if (errno)
            ...
            if (*dateString ==':') {
				...
				// Avoid the use of errno as it is not available on Windows CE
				if (dateString == newPosStr || second == LONG_MIN || second == LONG_MAX)
					return NaN;
#else
                second = strtol(dateString, &newPosStr, 10);
                if (errno)
    ...
    if (*dateString) {
        ...
        if (*dateString == '+' || *dateString == '-') {
			...
			// Avoid the use of errno as it is not available on Windows CE
			if (dateString == newPosStr || o == LONG_MIN || o == LONG_MAX)
				return NaN;
#else
            long o = strtol(dateString, &newPosStr, 10);
            if (errno)
            ...
            int sgn = (o < 0) ? -1 : 1;
            o = abs((int)o);  // Changed by Paul Pedriana in order to work around a  bug.
            if (*dateString != ':') {
				...
				// Avoid the use of errno as it is not available on Windows CE
				if (dateString == newPosStr || o2 == LONG_MIN || o2 == LONG_MAX)
					return NaN;
#else
				long o2 = strtol(dateString, &newPosStr, 10);
				if (errno)
    ...
    if (*dateString && year == -1) {
		...
		year = strtol(dateString, &newPosStr, 10);
		// Avoid the use of errno as it is not available on Windows CE
		if (dateString == newPosStr || year == LONG_MIN || year == LONG_MAX)
			return NaN;
#else
        year = strtol(dateString, &newPosStr, 10);
        if (errno)
