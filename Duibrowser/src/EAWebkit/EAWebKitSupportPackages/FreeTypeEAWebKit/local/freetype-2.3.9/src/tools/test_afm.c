...
#include FT_CONFIG_STANDARD_LIBRARY_H
  ...
  void dump_fontinfo( AFM_FontInfo  fi )
  {
    ...
    FT_PRINTF( "This AFM is for %sCID font.\n\n",
            ( fi->IsCIDFont ) ? "" : "non-" );

    FT_PRINTF( "FontBBox: %.2f %.2f %.2f %.2f\n", fi->FontBBox.xMin / 65536.,
                                               fi->FontBBox.yMin / 65536.,
                                               fi->FontBBox.xMax / 65536.,
                                               fi->FontBBox.yMax / 65536. );
    FT_PRINTF( "Ascender: %.2f\n", fi->Ascender / 65536. );
    FT_PRINTF( "Descender: %.2f\n\n", fi->Descender / 65536. );

    if ( fi->NumTrackKern )
      FT_PRINTF( "There are %d sets of track kernings:\n",
              fi->NumTrackKern );
    else
      FT_PRINTF( "There is no track kerning.\n" );
    ...
    for ( i = 0; i < fi->NumTrackKern; i++ )
    {
      ...
      FT_PRINTF( "\t%2d: %5.2f %5.2f %5.2f %5.2f\n", tk->degree,
                                                  tk->min_ptsize / 65536.,
                                                  tk->min_kern / 65536.,
                                                  tk->max_ptsize / 65536.,
                                                  tk->max_kern / 65536. );
    ...
    if ( fi->NumKernPair )
      FT_PRINTF( "There are %d kerning pairs:\n",
              fi->NumKernPair );
    else
      FT_PRINTF( "There is no kerning pair.\n" );
