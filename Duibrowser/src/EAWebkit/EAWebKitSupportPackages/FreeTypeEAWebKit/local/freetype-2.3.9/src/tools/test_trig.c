...
#define  THRESHOLD  64
  ...
  test_cos( void )
  {
    ...
    for ( i = 0; i < FT_ANGLE_2PI; i += 0x10000 )
    {
      ...
      {
        error = 1;
        FT_PRINTF( "FT_Cos[%3d] = %.7f  cos[%3d] = %.7f\n",
  ...
  test_sin( void )
  {
    ...
    for ( i = 0; i < FT_ANGLE_2PI; i += 0x10000 )
    {
      ...
      {
        error = 1;
        FT_PRINTF( "FT_Sin[%3d] = %.7f  sin[%3d] = %.7f\n",
                (i >> 16), f1/65536.0, (i >> 16), d2 );
      }
    }
  }
  ...
  test_tan( void )
  {
    ...
    for ( i = 0; i < FT_ANGLE_PI2-0x2000000; i += 0x10000 )
    {
      ...
      {
        error = 1;
        FT_PRINTF( "FT_Tan[%3d] = %.7f  tan[%3d] = %.7f\n",
                (i >> 16), f1/65536.0, (i >> 16), d2 );
      }
    }
  }
  ...
  test_atan2( void )
  {
    ...
    for ( i = 0; i < FT_ANGLE_2PI; i += 0x10000 )
    {
      ...
      if ( abs( i - j ) > 1 )
      {
        FT_PRINTF( "FT_Atan2( %.7f, %.7f ) = %.5f, atan = %.5f\n",
  ...
  test_unit( void )
  {
    ...
    for ( i = 0; i < FT_ANGLE_2PI; i += 0x10000 )
    {
      ...
      {
        error = 1;
        FT_PRINTF( "FT_Vector_Unit[%3d] = ( %.7f, %.7f )  vec = ( %.7f, %.7f )\n",
                (i >> 16),
  ...
  test_length( void )
  {
    ...
    for ( i = 0; i < FT_ANGLE_2PI; i += 0x10000 )
    {
      ...
      {
        error = 1;
        FT_PRINTF( "FT_Length( %.7f, %.7f ) = %.5f, length = %.5f\n",
  ...
  test_rotate( void )
  {
    ...
    for ( rotate = 0; rotate < FT_ANGLE_2PI; rotate += 0x10000 )
    {
      ...
      for ( i = 0; i < FT_ANGLE_2PI; i += 0x10000 )
      {
        ...
        {
          error = 1;
          FT_PRINTF( "FT_Rotate( (%.7f,%.7f), %.5f ) = ( %.7f, %.7f ), rot = ( %.7f, %.7f )\n",
                  c1, s1, ra,
  ...
  int main( void )
  {
    ...
    if (!error)
      FT_PRINTF( "trigonometry test ok !\n" );
