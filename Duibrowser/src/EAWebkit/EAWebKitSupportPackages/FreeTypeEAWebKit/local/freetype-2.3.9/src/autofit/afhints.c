...
#define AF_INDEX_NUM( ptr, base )  ( (ptr) ? ( (ptr) - (base) ) : -1 )
  ...
  af_glyph_hints_dump_points( AF_GlyphHints  hints )
  {
    ...
    FT_PRINTF( "Table of points:\n" );
    FT_PRINTF(   "  [ index |  xorg |  yorg |  xscale |  yscale "
              "|  xfit  |  yfit  |  flags ]\n" );

    for ( point = points; point < limit; point++ )
    {
      FT_PRINTF( "  [ %5d | %5d | %5d | %-5.2f | %-5.2f "
  ...
  af_glyph_hints_dump_segments( AF_GlyphHints  hints )
  {
    ...
    for ( dimension = 1; dimension >= 0; dimension-- )
    {
      ...
      FT_PRINTF ( "Table of %s segments:\n",
               dimension == AF_DIMENSION_HORZ ? "vertical" : "horizontal" );
      FT_PRINTF ( "  [ index |  pos  |  dir  | link | serif |"
               " height  | extra | flags    ]\n" );

      for ( seg = segments; seg < limit; seg++ )
      {
        FT_PRINTF ( "  [ %5d | %5.2g | %5s | %4d | %5d | %5d | %5d | %s ]\n",
                 seg - segments,
  ...
  af_glyph_hints_dump_edges( AF_GlyphHints  hints )
  {
    ...
    for ( dimension = 1; dimension >= 0; dimension-- )
    {
       ...
       */
      FT_PRINTF ( "Table of %s edges:\n",
               dimension == AF_DIMENSION_HORZ ? "vertical" : "horizontal" );
      FT_PRINTF ( "  [ index |  pos  |  dir  | link |"
               " serif | blue | opos  |  pos  | flags   ]\n" );

      for ( edge = edges; edge < limit; edge++ )
      {
        FT_PRINTF ( "  [ %5d | %5.2g | %5s | %4d |"
                 " %5d |   %c  | %5.2f | %5.2f | %s ]\n",
                 edge - edges,
                 (int)edge->opos / 64.0,
