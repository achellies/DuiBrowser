...
QByteArray QIsciiCodec::convertFromUnicode(const ushort* uc, int len, ConverterState* state)
{
    char replacement = '?';
    bool halant = false;
    if (state) {
...
QString QIsciiCodec::convertToUnicode(const char* chars, int len, ConverterState *state)
{
    bool halant = false;
    if (state) {
		halant = !!state->state_data[0];
